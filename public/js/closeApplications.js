  ...
  if (closeBtn.getAttribute('data-status') === 'closed') {
    try {
      ...
      if (res.data.status === 'success') {
        showToast(
          'Applications Opened for this job',
          'bottom',
          'right',
          '#1da977'
        );
        ...
        closeBtn.setAttribute('data-status', 'open');
        closeBtn.innerHTML =
          "<i class='far fa-stop-circle' style='margin-right: 0.4rem'></i> Close Applications";
      }
    } catch (err) {
      console.log(err);
    ...
    try {
      ...
      if (res.data.status === 'success') {
        showToast(
          'Applications Closed for this job',
          'bottom',
          'right',
          '#1da977'
        );
        ...
        document.getElementById(`settings-${jobId}`).classList.toggle('show');
        closeBtn.setAttribute('data-status', 'closed');
        closeBtn.innerHTML =
          "<i class='far fa-stop-circle' style='margin-right: 0.4rem'></i> Open Applications";
      }
    } catch (err) {
      console.log(err);
      showToast(`${err.response.data.message}`, 'bottom', 'right', '#c0392b');
